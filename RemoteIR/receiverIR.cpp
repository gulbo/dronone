 ...
 */
int ReceiverIR::receive(RemoteIR::Format *format, uint8_t *buf, int bufsiz, int timeout) {
    int cnt = 0;
    while (getState() != Received) {
        cnt++;
        if (timeout < cnt) {
            return -1;
        }
    }
    return getData(format, buf, bufsiz * 8);
}

/** MADE BY US 
 * @return type long decoded data
 *        -1 if nothing
 */
long ReceiverIR::decode_data(){
    RemoteIR::Format format_temp;
    int bitcount;
    uint8_t buffer[32];
    bitcount = receive(&format_temp, buffer, sizeof(buffer)); //#bits read
    if (bitcount>0){ //if received something
        const int n = bitcount / 8 + (((bitcount % 8) != 0) ? 1 : 0); //#bytes of the read data
        long result;
        for (int i = 0; i < n; i++) {
